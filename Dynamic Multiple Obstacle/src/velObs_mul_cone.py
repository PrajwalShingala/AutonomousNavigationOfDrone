...
def getOptimal_velocity(drone_pos, drone_vel, obs_pos, obs_vel, eng, R=2.0):
	#print(R)
	## Configuration space radius(IMP)
	#R = 2;

	#print drone_pos.shape
	#print drone_vel.shape
	#print obs_pos.shape
	#print obs_vel.shape

	## obs_pos_np: it has current positions of all the obstacles
	## obs_vel_np: it has current velocity of all the obstacles
	## drone_pos_np: It has current position of drone
	## drone_vel_np: It has current velocity of drone
	## All necessary variables to be passed to the matlab optimization function
	drone_pos_matlab = matlab.double(np.ndarray.tolist(drone_pos))
	drone_vel_matlab = matlab.double(np.ndarray.tolist(drone_vel))
	obs_pos_matlab = matlab.double(np.ndarray.tolist(obs_pos))
	obs_vel_matlab = matlab.double(np.ndarray.tolist(obs_vel))

	## Defining matlab workspace variables
	eng.workspace['drone_pos_matlab'] = drone_pos_matlab
	eng.workspace['drone_vel_matlab'] = drone_vel_matlab
	eng.workspace['obs_pos_matlab'] = obs_pos_matlab
	eng.workspace['obs_vel_matlab'] = obs_vel_matlab
	eng.workspace['R'] = R

	## Let's run velocity obstacle
	start_time = time.time()
	new_vel = np.array(eng.eval('velocity_obstacle(drone_pos_matlab, drone_vel_matlab, obs_pos_matlab, obs_vel_matlab, R)')).reshape(2,1)
	#print time.time() - start_time
	#print 'New velocityyyyyy: ', new_vel
	return new_vel
