	...
	if tag_odom.detections:
		...
		for tag_val in tag_odom.detections:
			...
			if not tag_val.id in id_dict:
				...
				rel_vel_x = np.zeros((length,1)) #vel_x = [0] * len(x_tag_previous) #np.zeros_like(x_tag_previous)
				rel_vel_y = np.zeros((length,1)) #vel_y = [0] * len(x_tag_previous) #np.zeros_like(x_tag_previous)
				rel_vel_z = np.zeros((length,1)) #vel_z = [0] * len(x_tag_previous) #np.zeros_like(x_tag_previous)
				...
				if not initialised[index]:
					...
					rel_dist_trav_x = x_tag_new[index,0] - x_tag_previous[index]
					rel_dist_trav_y = y_tag_new[index,0] - y_tag_previous[index]
					rel_dist_trav_z = z_tag_new[index,0] - z_tag_previous[index]
					...
					rel_vel_x[index,0] = rel_dist_trav_x/time_elapsed
					if abs(rel_vel_x[index,0]) < 0.05:
						rel_vel_x[index,0] = 0.0
					rel_vel_y[index,0] = rel_dist_trav_y/time_elapsed
					if abs(rel_vel_y[index,0]) < 0.05:
						rel_vel_y[index,0] = 0.0 

					rel_vel_z[index,0] = rel_dist_trav_z/time_elapsed
					if abs(rel_vel_z[index,0]) < 0.05:
						rel_vel_z[index,0] = 0.0 

					rel_vel_x = np.array(rel_vel_x)
					rel_vel_y = np.array(rel_vel_y)
					rel_vel_z = np.array(rel_vel_z)
					...
					if initialised[index] and time_since_last_action > 2.0:
						...
						# print vel_x, vel_y
							curr_pos = [drone_odom.pose.pose.position.x,drone_odom.pose.pose.position.y]
							curr_vel_in = [0,0]
							curr_pos = [0,0]


							del_new_vel = getOptimal_velocity(np.array([curr_pos]),
															 np.array(curr_vel_in),
															  np.array([x_tag_new,y_tag_new]).T, np.array([rel_vel_x, rel_vel_y]).T, eng, 4.0)
							
							last_time_compute = time.time()

							curr_vel = np.array([curr_vel]).reshape(1,2)

							new_vel = np.array([curr_vel]).reshape(1,2) + np.array([del_new_vel]).reshape(1,2)
							print curr_vel
							print del_new_vel
							print new_vel[0,0]
							print new_vel[0,1]
							print "upar wali thi new vel"

							if abs(del_new_vel[0]) < 0.05 and abs(del_new_vel[1]) < 0.05:
								vel_flag = 0
							else:
							...
							publ_cone(curr_vel[0,0], curr_vel[0,1], new_vel[0,0], new_vel[0,1], vel_flag)
