...
from velObs_mul import checkCollision
...
def callback(tag_odom, drone_odom):
	...
	#print 'Nuumber of obstacles areeeeeeee: ', len(tag_odom.detections)
		...
		for tag_val in tag_odom.detections:
			...
			if not tag_val.id in id_dict:
				...
				if not initialised[index]:
					...
					if time_stamp_new[index] - timeOfLastPublish[index] > 1 :
					...
					if initialised[index]:
						...
						print flag'''
						print 'Printinggggggggg index:', index
